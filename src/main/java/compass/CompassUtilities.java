...
;
	...
	private static final List<String> toolTipsText = Arrays.asList(
		...
		"View, without SCHEMABINDING"+tttSeparator+"PostgreSQL only supports views with the equivalent of WITH SCHEMABINDING, i.e. a table cannot be dropped if a view depends on it. A view without the SCHEMABINDING clause will still be created by Babelfish, but as if WITH SCHEMABINDING was specified. However, weak schemabinding can be enabled via the GUC 'babelfishpg_tsql.weak_view_binding'. When this GUC is ON, views created without SCHEMABINDING clause will have weak schemabinding, while views explicitly created WITH SCHEMABINDING maintain strong binding regardless of the GUC setting.",
